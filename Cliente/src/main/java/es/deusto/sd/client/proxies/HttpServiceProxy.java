...
public class HttpServiceProxy implements IStravaServiceProxy {
	...
	public List<Category> getAllCategories() {
		try {
			HttpRequest request = HttpRequest.newBuilder().uri(URI.create(BASE_URL + "/auctions/categories"))
			...
			default ->
				throw new RuntimeException("Failed to fetch categories with status code: " + response.statusCode());
			};
	...
	public List<Article> getArticlesByCategory(String categoryName, String currency) {
		try {
			...
			HttpRequest request = HttpRequest
					.newBuilder().uri(URI.create(BASE_URL + "/auctions/categories/" + encodedCategoryName
							+ "/articles?currency=" + currency))
					.header("Content-Type", "application/json").GET().build();
			...
			default ->
				throw new RuntimeException("Failed to fetch articles with status code: " + response.statusCode());
			};
		} catch (IOException | InterruptedException e) {
	...
	public Article getArticleDetails(Long articleId, String currency) {
		try {
			...
			default -> throw new RuntimeException(
					"Failed to fetch article details with status code: " + response.statusCode());
			};
		} catch (IOException | InterruptedException e) {
	...
	public void makeBid(Long articleId, Float amount, String currency, String token) {
		try {
			HttpRequest request = HttpRequest.newBuilder()
					.uri(URI.create(BASE_URL + "/auctions/articles/" + articleId + "/bid?amount=" + amount
							+ "&currency=" + currency))
...
=======
			HttpRequest request = HttpRequest.newBuilder().uri(URI.create(BASE_URL + "/api/sesion/sesiones/recientes"))
					.header("Content-Type", "application/json").GET().build();
>>>>>>> a153637e46614a272db9d6ab6ef6753033a6a859
