...
public class MetaServer {
	...
	public static void main(String args[]) {
		...
		try (ServerSocket tcpServerSocket = new ServerSocket(serverPort);) {
			...
			System.out
					.println(" - MetaServer: esperando conexiones '" + tcpServerSocket.getInetAddress().getHostAddress()
							+ ":" + tcpServerSocket.getLocalPort() + "' ...");

			while (true) {
				new MetaService(tcpServerSocket.accept());
				System.out.println(
						" - MetaServer: Nueva conexión de cliente aceptada. Número de cliente: " + ++numClients);
			}
		} catch (IOException e) {
